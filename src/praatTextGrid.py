...
def openTextGridForWavFile(wavFileName, textGridFileName = None):
	"""
	tries to open, read and return the Praat TextGrid annotation
	for the given WAV file. if the text grid is not found, an error is thrown
	:param wavFileName: a full file name, including the path
	:param textGridFileName: the name of the text grid to be opened. This is
		by default set to None, and the wav file name is used, replacing the
		suffix with "TextGrid"
	:return: a list of all tiers in the text grid
	"""
	path, fNameOnly, suffix = generalUtility.splitFullFileName(wavFileName)
	textGridFileName = "%s%s.TextGrid" % (path, fNameOnly)
	if not os.path.isfile(textGridFileName):
		raise Exception("no TextGrid found for file '%s'" % wavFileName)
	textGrid = PraatTextGrid(0, 0)
