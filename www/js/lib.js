var lib = (function(window) {
  ...
  function parseParameters(names, params) {
    ...
    if (params.length > 1) {
      throw new Error(
        "No more than one optional argument containing an object of key-value pairs expected."
      );
    } else if (params.length && !containsKeywords(params)) {
      throw new Error(
        "Argument must be an object of key-value pairs that act as function parameters."
      );
    }
      ...
      if (str.indexOf("@") !== -1) {
        throw new Error(
          names.join(".") +
            " invalid name: " +
            str +
            ", please specify versions and environments with [@version]"
        );
      }
  ...
  function executeRemote(cfg, names, params, callback) {
    formatParamsObjectAsync(params, function(err, params) {
      ...
      cfg.bg &&
        (pathname +=
          ":bg" +
          (typeof cfg.bg === "string" ? "=" + encodeURIComponent(cfg.bg) : ""));
        ...
        if (xhr.readyState === 4) {
          ...
          if (
            ...
            reader.addEventListener("error", function() {
              return callback(
                new Error("Could not read response"),
                response,
                resheaders
              );
            });
